...
\end{bmatrix}\]
* Loss function is 
\[\matchcal{L}(\hat{y}, y) = -\sum^4_{j = 1} y_j \log \hat{y}_j\]
...
Cost on entire training set:
    \[J(w^{[1]}, b^{[1]}, \ldots) = \frac{1}{m} \sum^m_{i = 1}\matchcal{L}(\hat{y}^{(i)}, y^{(i)}) \]
...
* Forward prop: $z^{[L]} \rightarrow a^{[L]} = \hat{y} \rightarrow \matchcal{L}(\hat{y}, y)$
