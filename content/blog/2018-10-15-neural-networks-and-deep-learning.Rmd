...
* Hidden layer(s) generates activations. The first hidden layer, for example, with 4 nodes, is denoted $a^{[1]} = \{a_1^{[1]}, a_2^{[1]}, a_3^{[1]},  a_{4}^{[1]}\}$.  Called __hidden__ as don't see true values in training set.  First hidden layer will have associated paramaters, for example when $X \in \mathbb{R}^{1 \times 3}$ and $a^{[1]} \in \mathbb{R}^{4 \times 1}$ then will have parameters
...
So no matter how many layers, will just be computing a linear activtation function, so may as well not have any hidden layers.
...
* $n_x = n^{[0]}$ is the number of input parametesr
...
* Compute derivate $dW^{[1]} = \frac{dJ}{dw^{[1]}}$, $db^{[1]} = \frac{dJ}{db^{[1]}}$, $\ldots$
* Update phase 
